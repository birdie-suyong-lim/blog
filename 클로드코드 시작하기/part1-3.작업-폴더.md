...
> 🎨 **이미지 프롬프트**
> 깔끔한 플랫 벡터 일러스트, 부드러운 파스텔 톤에 옅은 청록(teal) 포인트, 흰 배경. 여러 개의 방(폴더)이 나란히 있고, 그중 '하나의 방(폴더)' 안에만 친근한 로봇 비서가 들어가 그 방의 서류·파일을 정리하는 모습. 다른 방들은 닫혀 있어 손대지 않는 느낌. '지정한 폴더 안에서만 일한다'는 개념이 한눈에 보이게. 이미지 안에 글자/텍스트는 절대 넣지 말 것.
